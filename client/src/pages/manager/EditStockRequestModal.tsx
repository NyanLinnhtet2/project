...
import { X, Loader2, PenSquare, Package } from "lucide-react";
      ...
      <div className="w-full max-w-md rounded-3xl bg-white p-4 sm:p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="mt-4">
          ...
          <textarea
            className="w-full rounded-xl border border-slate-200 px-4 py-3 outline-none transition focus:border-blue-500 focus:shadow-md"
            rows={3}
            placeholder="e.g., Physical count mismatch, Damaged items, Recount needed..."
            value={reason}
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-slate-200 mt-4">
          ...
          <button
            ...
            disabled={isLoading}
            className="flex-1 rounded-xl bg-linear-to-r from-blue-600 to-blue-700 py-3 font-medium text-white shadow-lg shadow-blue-200 transition hover:scale-105 hover:shadow-xl hover:shadow-blue-300 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isLoading ? (
